...
if __name__=="__main__":
    ...
    for w in wojewodztwa:
        ...
        pogoda = c.execute("""SELECT max_temp,min_temp,cisnienie,zachod-wschod as godzin_dnia,phase,wypadki
                                FROM pogoda JOIN wypadki ON wypadki.data=pogoda.data
                                JOIN moon_phase ON moon_phase.data=pogoda.data
                            WHERE pogoda.data>'2009-01-01' AND pogoda.wojewodztwo=wypadki.wojewodztwo and pogoda.wojewodztwo=%d"""%w[0]).fetchall()
        print len(pogoda)
        ...
        with open('pogoda_%s.csv'%w[1],'w') as file:
